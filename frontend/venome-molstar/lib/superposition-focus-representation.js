...
const SuperpositionFocusRepresentationParams = (plugin) => {
    const reprParams = StateTransforms.Representation.StructureRepresentation3D
        .definition.params(void 0, plugin);
    return {
...
export var SuperpositionFocusRepresentationTags;
...
})(SuperpositionFocusRepresentationTags || (SuperpositionFocusRepresentationTags = {}));
const TagSet = new Set([
    SuperpositionFocusRepresentationTags.SurrSel,
    SuperpositionFocusRepresentationTags.SurrRepr,
]);
class SuperpositionFocusRepresentationBehavior extends PluginBehavior.WithSubscribers {
    get surrLabel() {
    ...
    ensureShape(cell) {
        ...
        if (!refs[SuperpositionFocusRepresentationTags.SurrSel]) {
            ...
            }, { tags: SuperpositionFocusRepresentationTags.SurrSel }).ref;
        }
        // Representations
    ...
    clear(root) {
        ...
        const surrs = state.select(StateSelection.Generators.byRef(root)
            .subtree()
        ...
        for (const s of surrs) {
            update
                .to(s)
                .update(StateTransforms.Model.StructureSelectionFromExpression, (old) => ({ ...old, expression }));
        }
