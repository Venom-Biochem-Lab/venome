...
export async function superpositionExportHierarchy(
...
function _superpositionExportHierarchy(
	plugin: PluginContext,
	options?: { format?: "cif" | "bcif" }
) {
	return Task.create("Export", async (ctx) => {
		...
		const structures =
			superpositionState.loadedStructs[segmentIndex].slice();
		...
		for (const molId of structures) {
			...
			const _s: any =
				plugin.managers.structure.hierarchy.current.refs.get(modelRef!);
			...
			const name = entryMap.has(s.model.entryId)
				? `${s.model.entryId}_${
						entryMap.get(s.model.entryId)! + 1
				  }.${format}`
				: `${s.model.entryId}.${format}`;
			entryMap.set(
				s.model.entryId,
				(entryMap.get(s.model.entryId) ?? 0) + 1
			);
