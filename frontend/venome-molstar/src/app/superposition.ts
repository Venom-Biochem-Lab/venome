...
function getRandomColor(plugin: PluginContext, segmentIndex: number) {
	...
	if (!spState)
		throw new Error(
			"customState.superpositionState has not been initialized"
		);
...
export async function initSuperposition(
	plugin: PluginContext,
	completeSubject?: Subject<boolean>
) {
	let success = false;
	try {
		await plugin.clear();
		...
		const afStrUrls = await getAfUrl(
			plugin,
...
async function getAfUrl(plugin: PluginContext, accession: string) {
	...
	await plugin.runTask(
		Task.create("Get AlphaFold URL", async (ctx) => {
			try {
...
export async function loadAfStructure(plugin: PluginContext) {
	...
	const { structure } = await loadStructure(
		plugin,
		customState.superpositionState.alphafold.apiData.cif,
		"mmcif",
		false
	);
	...
	const chainSel = await plugin.builders.structure.tryCreateComponentStatic(
		strInstance,
		"polymer",
		{
...
export async function superposeAf(
	plugin: PluginContext,
	traceOnly: boolean,
	segmentIndex?: number
) {
	...
	if (!afStrRef) return;
	const afStr: any = plugin.managers.structure.hierarchy.current.refs.get(
		afStrRef!
	);
	...
	if (!spState.alphafold.transforms[segmentNum]) {
		...
		segmentClusters.forEach((cluster: any) => {
			const modelRef =
				spState.models[
					`${cluster[0].pdb_id}_${cluster[0].struct_asym_id}`
				];
			if (modelRef) {
				const structHierarchy: any =
					plugin.managers.structure.hierarchy.current.refs.get(
						modelRef!
					);
				if (structHierarchy) {
					const input = [structHierarchy.components[0], afStr];
					const structures = input.map((s) => s.cell.obj?.data!);
					let { entries, failedPairs, zeroOverlapPairs } =
						alignAndSuperposeWithSIFTSMapping(structures, {
							traceOnly,
							includeResidueTest: (loc) =>
								StructureProperties.atom.B_iso_or_equiv(loc) >
								70,
							applyTestIndex: [1],
						});
					...
					if (entries && entries[0]) {
						...
						coordinateSystems.push(
							input[0]?.transform?.cell.obj?.data.coordinateSystem
						);
						...
						rmsdList.push(
							`${cluster[0].pdb_id} chain ${
								cluster[0].struct_asym_id
							}:${entries[0].transform.rmsd.toFixed(2)}`
						);
					} else {
		...
		if (mappingResult.length > 0) {
			...
			spState.alphafold.rmsds[segmentNum] = rmsdList.sort(
				(a: string, b: string) =>
					parseFloat(a.split(":")[1]) - parseFloat(b.split(":")[1])
			);
		}
	}
...
export async function renderSuperposition(
	plugin: PluginContext,
	segmentIndex: number,
	entryList: ClusterMember[]
) {
	const customState = PluginCustomState(plugin);
	const superpositionParams = customState.initParams!.superpositionParams;
	...
	return plugin.dataTransaction(async () => {
		...
		for await (const s of entryList) {
			...
			spState.loadedStructs[segmentIndex].push(
				`${s.pdb_id}_${s.struct_asym_id}`
			);

			// Set Coordinate server url
			const request: ModelServerRequest =
				superpositionParams && superpositionParams.ligandView
			...
			if (
				...
				const polymerInstance = plugin.state.data.select(
					spState.entries[s.pdb_id]
				)[0];
				modelRef = polymerInstance.transform.parent;
				const modelInstance = plugin.state.data.select(modelRef)[0];
				strInstance = await plugin.builders.structure.createStructure(
					modelInstance,
					{ name: "model", params: {} }
				);
				clearOnFail = false;
				...
				const isBinary =
					customState.initParams.encoding === "bcif" ? true : false;
				const { model, structure } = await loadStructure(
					plugin,
					strUrl,
					"mmcif",
					isBinary
			...
			const matrix = Mat4.ofRows(
				customState.superpositionState.matrixData[
					`${s.pdb_id}_${s.auth_asym_id}`
				].matrix
			);
			...
			if (
				...
				chainSel =
					await plugin.builders.structure.tryCreateComponentFromExpression(
						strInstance,
						chainSelection(s.struct_asym_id),
						`Chain-${segmentIndex}`,
						{ label: `Chain`, tags: [`superposition-sel`] }
					);
				if (chainSel) {
					await plugin.builders.structure.representation.addRepresentation(
						chainSel,
			...
			if (superpositionParams && superpositionParams.ligandView) {
				...
				const hetInfo = await getLigandNamesFromModelData(
					plugin,
					state,
					modelRef
				);
				...
				if (carbEntityCount > 0) {
					...
					const allCarbPolymers = await getCarbPolymerDetailsFromApi(
						plugin,
						s.pdb_id
					);

					// Polymer chain + surroundings query
					const polymerChainWithSurroundings =
					...
					for (const carbEntityChainId of allCarbPolymers.branchedChains) {
						...
						const carbEntityChainInVicinity =
							MS.struct.filter.intersectedBy({
								0: polymerChainWithSurroundings,
								by: carbEntityChain,
							});

						const data = plugin.state.data.select(
							strInstance.ref
						)[0].obj.data;
						const carbChainSel = Script.getStructureSelection(
							carbEntityChainInVicinity,
							data
						);
				...
				if (invalidStruct) {
					spState.invalidStruct.push(
						`${s.pdb_id}_${s.struct_asym_id}`
					);
					const loadedStructIndex = spState.loadedStructs[
						segmentIndex
					].indexOf(`${s.pdb_id}_${s.struct_asym_id}`);
					if (loadedStructIndex > -1)
						spState.loadedStructs[segmentIndex].splice(
							loadedStructIndex,
							1
						);
...
async function getLigandNamesFromModelData(
	plugin: PluginContext,
	state: State,
	modelRef: string
) {
...
async function getMatrixData(plugin: PluginContext) {
	...
	const matrixAccession =
		customState.initParams.superpositionParams?.matrixAccession ??
		customState.initParams.moleculeId;
	...
	try {
		const clusterRecData = await plugin.runTask(
			assetManager.resolve(clusterRecUrl, "json", false)
		);
...
async function getSegmentData(plugin: PluginContext) {
	...
	try {
		const result = await plugin.runTask(
			assetManager.resolve(url, "json", false)
		);
		if (result?.data) {
...
function getChainLigands(carbEntity: any) {
	...
	for (const chain of carbEntity.chains) {
		...
		for (const residue of chain.residues) {
			chainLigands.push(
				residue.chem_comp_id + "-" + residue.residue_number
			);
		}
...
async function getCarbPolymerDetailsFromApi(
	plugin: PluginContext,
	pdb_id: string
) {
	const customState = PluginCustomState(plugin);
	...
	try {
		...
		if (result && result.data) {
			...
			for (const carbEntity of carbEntities) {
				...
				branchedlabels = branchedlabels.concat(
					carbLigData.ligandLabels
				);
			}
		}
	} catch (e) {}
