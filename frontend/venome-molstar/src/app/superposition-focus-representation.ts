...
const SuperpositionFocusRepresentationParams = (plugin: PluginContext) => {
	const reprParams = StateTransforms.Representation.StructureRepresentation3D
		.definition.params!(void 0, plugin) as PD.Params;
	return {
		...
		surroundingsParams: PD.Group(reprParams, {
			...
			customDefault: createStructureRepresentationParams(plugin, void 0, {
				...
				sizeParams: { scale: 0.3 },
			}),
		}),
	};
};

type SuperpositionFocusRepresentationProps = PD.ValuesFor<
	ReturnType<typeof SuperpositionFocusRepresentationParams>
>;
...
const TagSet: Set<SuperpositionFocusRepresentationTags> = new Set([
	SuperpositionFocusRepresentationTags.SurrSel,
	SuperpositionFocusRepresentationTags.SurrRepr,
]);
...
class SuperpositionFocusRepresentationBehavior extends PluginBehavior.WithSubscribers<SuperpositionFocusRepresentationProps> {
	...
	private ensureShape(
		...
		const refs = StateSelection.findUniqueTagsInSubtree(
			tree,
			cell.transform.ref,
			TagSet
		);
		...
		if (!refs[SuperpositionFocusRepresentationTags.SurrSel]) {
			refs[SuperpositionFocusRepresentationTags.SurrSel] = builder
				...
				.apply(
					...
					{ tags: SuperpositionFocusRepresentationTags.SurrSel }
				).ref;
		}
		...
		if (!refs[SuperpositionFocusRepresentationTags.SurrRepr]) {
			refs[SuperpositionFocusRepresentationTags.SurrRepr] = builder
				...
				.apply(
					...
					this.params.surroundingsParams,
	...
	private clear(root: StateTransform.Ref) {
		...
		const surrs = state.select(
			StateSelection.Generators.byRef(root)
				.subtree()
				.withTag(SuperpositionFocusRepresentationTags.SurrSel)
		);
	...
	async update(params: SuperpositionFocusRepresentationProps) {
		...
		for (const repr of state.select(
			all.withTag(SuperpositionFocusRepresentationTags.SurrRepr)
		)) {
		...
		if (params.expandRadius !== old.expandRadius)
			await this.clear(StateTransform.RootRef);
