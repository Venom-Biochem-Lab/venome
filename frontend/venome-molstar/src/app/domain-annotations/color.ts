...
export function DomainAnnotationsColorTheme(
	...
	if (
		...
		ctx.structure.models[0].customProperties.has(
			DomainAnnotationsProvider.descriptor
		)
	) {
	...
	return {
		...
		props: props,
		description:
			"Highlights Sequnece and Structure Domain Annotations. Data obtained via PDBe.",
	};
}

export const DomainAnnotationsColorThemeProvider: ColorTheme.Provider<
	Params,
	"pdbe-domain-annotations"
> = {
		...
		domainTypes.forEach((tp, index) => {
			if (domainNames[index].length > 0) {
				optionObj[tp as string] = PD.Group(
					{
						kind: PD.Select(
							domainNames[index][0] as string,
							PD.arrayToOptions(domainNames[index] as string[])
						),
					},
					{ isFlat: true }
				);
	...
	isApplicable: (ctx: ThemeDataContext) =>
		DomainAnnotations.isApplicable(ctx.structure?.models[0]),
	ensureCustomProperties: {
		attach: (ctx: CustomProperty.Context, data: ThemeDataContext) => {
			return data.structure
				? DomainAnnotationsProvider.attach(
						ctx,
