...
namespace DomainAnnotations {
	...
	export async function fromServer(
		...
		model: Model,
		props: DomainAnnotationsProps
	): Promise<CustomProperty.Data<DomainAnnotations>> {
...
export const DomainAnnotationsProvider: CustomModelProperty.Provider<
...
> = CustomModelProperty.createProvider({
	...
	obtain: async (
		ctx: CustomProperty.Context,
		data: Model,
		props: Partial<DomainAnnotationsProps>
	) => {
...
function createdomainMapFromJson(
	modelData: Model,
	data: any
): DomainAnnotations["data"] | undefined {
	...
	for (const db_name of Object.keys(data)) {
		...
		for (const db_code of Object.keys(db)) {
			...
			for (const map of domain.mappings) {
				...
				for (
					let i = map.start.residue_number;
					i <= map.end.residue_number;
					i++
				) {
					const seq_id = i;
					const idx = findResidue(
						modelData,
						indexMap,
						map.entity_id + "",
						map.chain_id,
