...
import {
	ElementIndex,
	ResidueIndex,
} from "molstar/lib/mol-model/structure/model/indexing";
...
interface IndexEntry {
	...
	pivots: {
		[i: number]:
			| [unit: Unit.Atomic, start: ElementIndex, end: ElementIndex]
			| undefined;
	};
}
	...
	for (const unit of structure.units) {
		...
		while (chainsIt.hasNext) {
			...
			while (residuesIt.hasNext) {
				...
				loc.element = (
					traceElement >= 0 ? traceElement : start
				) as ElementIndex;
				if (!includeTest(loc, rI, start, end)) continue;

				const key = `${dbName[rI]}-${accession[rI].split("-")[0]}-${
					num[rI]
				}`;
