...
import {
	...
	RCSBAssemblySymmetryConfig,
} from "molstar/lib/extensions/rcsb/assembly-symmetry/behavior";
...
export class PDBeMolstarPlugin {
	...
	static initParamsFromHtmlAttributes(
		element: HTMLElement
	): Partial<InitParams> {
	...
	async render(target: string | HTMLElement, options: Partial<InitParams>) {
		console.debug(
			"Rendering PDBeMolstarPlugin instance with options:",
			options
		);
		// Validate options
		if (!options) {
			console.error(
				"Missing `options` argument to `PDBeMolstarPlugin.render"
			);
			return;
		}
	...
	getMoleculeSrcUrl() {
		...
		if (this.initParams.moleculeId) {
			const request: Required<ModelServerRequest> = {
		...
		throw new Error(
			`Mandatory parameters missing! (customData or moleculeId must be defined)`
		);
	}
		...
		await runWithProgressMessage(this.plugin, progressMessage, async () => {
			...
			try {
				...
				if (!isHetView) {
					...
					if (
						this.initParams.hideStructure.length > 0 ||
					...
					const model =
						await this.plugin.builders.structure.createModel(
							trajectory
						);
					await this.plugin.builders.structure.createStructure(
						model,
						{ name: "model", params: {} }
					);
				}
				...
				if (this.initParams.loadMaps) {
					...
					const defaultMapParams =
						InitVolumeStreaming.createDefaultParams(
							asm,
							this.plugin
						);
	...
	applyVisualParams = () => {
		...
		for (const compGroup of componentGroups) {
			...
			const remove = this.initParams.hideStructure.some((type) =>
				StructureComponentTags[type]?.includes(tag)
			);
			if (remove) {
	...
	canvas = {
		...
		applySettings: async (settings?: {
			...
			if (settings.color) {
				renderer.backgroundColor = Color.fromRgb(
					settings.color.r,
					settings.color.g,
					settings.color.b
				);
	...
	visual = {
		...
		update: async (options: Partial<InitParams>, fullLoad?: boolean) => {
			console.debug(
				"Updating PDBeMolstarPlugin instance with options:",
				options
			);
			// Validate options
			if (!options) {
				console.error(
					"Missing `options` argument to `PDBeMolstarPlugin.visual.update"
				);
				return;
			}
			const validationIssues = validateInitParams(options);
			...
			this.plugin.config.set(
				PluginConfig.Viewport.ShowExpand,
				!this.initParams.hideCanvasControls.includes("expand")
			);
			this.plugin.config.set(
				PluginConfig.Viewport.ShowSelectionMode,
				!this.initParams.hideCanvasControls.includes("selection")
			);
			this.plugin.config.set(
				PluginConfig.Viewport.ShowAnimation,
				!this.initParams.hideCanvasControls.includes("animation")
			);
			this.plugin.config.set(
				PluginConfig.Viewport.ShowControls,
				!this.initParams.hideCanvasControls.includes("controlToggle")
			);
			this.plugin.config.set(
				PluginConfig.Viewport.ShowSettings,
				!this.initParams.hideCanvasControls.includes("controlInfo")
			);
			...
			for (const visual in data) {
				const tags =
					StructureComponentTags[
						visual as keyof typeof StructureComponentTags
					] ?? [];
					...
					if (componentRef) {
						...
						if (compVisual && compVisual.obj) {
							...
							if (data[visual] !== currentlyVisible) {
								await PluginCommands.State.ToggleVisibility(
									this.plugin,
									{ state: this.state, ref: componentRef }
								);
							}
						}
					}
		...
		setColor: async (param: {
			...
			if (param.highlight) {
				...
				marking.highlightEdgeColor = Color.darken(
					this.normalizeColor(param.highlight),
					1
				);
			...
			if (param.select) {
				...
				marking.selectEdgeColor = Color.darken(
					this.normalizeColor(param.select),
					1
				);
