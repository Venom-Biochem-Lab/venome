...
export function lociDetails(loci: Loci): EventDetail | undefined {
	switch (loci.kind) {
		...
		case "element-loci":
			return structureElementStatsDetail(
...
function structureElementStatsDetail(
...
function getElementDetails(
	...
	if (Unit.isAtomic(location.unit)) {
		elementDetails = atomicElementDetails(
			location as StructureElement.Location<Unit.Atomic>,
			granularity
		);
	} else if (Unit.isCoarse(location.unit)) {
		elementDetails = coarseElementDetails(
			location as StructureElement.Location<
...
function atomicElementDetails(
	...
	elementDetails["micro_het_comp_ids"] =
		granularity === "residue" && microHetCompIds.length > 1
			? microHetCompIds
			: ([elementDetails["comp_id"]] as any);

	return elementDetails;
}

function coarseElementDetails(
	...
	if (granularity === "residue") {
		if (elementDetails.seq_id_begin === elementDetails.seq_id_end) {
			...
			elementDetails["comp_id"] = seq.sequence.compId.value(
				elementDetails.seq_id_begin! - 1
			); // 1-indexed
		}
	}
...
export function _bundleLabel(
	bundle: Loci.Bundle<any>,
	granularity: LabelGranularity
) {
	let isSingleElements = true;
	for (const l of bundle.loci) {
		if (
			!StructureElement.Loci.is(l) ||
	...
	if (isSingleElements) {
		...
		const elementDetailsArr: EventDetail[] = locations.map((l) =>
			getElementDetails(l, granularity)
		);
		const atomIds: any = [
