...
export namespace PDBeVolumes {
	export function mapParams(
		...
		pdbeParams.options.channelParams["em"] = addDefaults(
			mapParams?.["em"],
			MAIN_MAP_DEFAULTS
		);
		pdbeParams.options.channelParams["2fo-fc"] = addDefaults(
			mapParams?.["2fo-fc"],
			MAIN_MAP_DEFAULTS
		);
		pdbeParams.options.channelParams["fo-fc(+ve)"] = addDefaults(
			mapParams?.["fo-fc(+ve)"],
			DIFF_MAP_DEFAULTS
		);
		pdbeParams.options.channelParams["fo-fc(-ve)"] = addDefaults(
	...
	export function toggle(plugin: PluginContext) {
		...
		const streamingState = state.select(
...
export namespace LigandView {
	export function query(ligandViewParams: LigandQueryParam): {
	...
	export function branchedQuery(params: any): {
		core: Expression.Expression;
		surroundings: Expression.Expression;
	} {
			...
			atmGroupsQueries.length === 1
				...
				: // Need to union before merge for fast performance
				  MS.struct.combinator.merge(
						atmGroupsQueries.map((q) =>
							MS.struct.modifier.union([q])
						)
				  ),
		]);
...
export namespace QueryHelper {
		...
		params.forEach((param) => {
			...
			if (param.entity_id)
				selection["entityTest"] = (l) =>
					StructureProperties.entity.id(l.element) ===
					param.entity_id;

			// chain
			if (param.struct_asym_id) {
				selection["chainTest"] = (l) =>
					StructureProperties.chain.label_asym_id(l.element) ===
					param.struct_asym_id;
			} else if (param.auth_asym_id) {
				selection["chainTest"] = (l) =>
					StructureProperties.chain.auth_asym_id(l.element) ===
					param.auth_asym_id;
			}
			...
			if (param.label_comp_id) {
				selection["residueTest"] = (l) =>
					StructureProperties.atom.label_comp_id(l.element) ===
					param.label_comp_id;
			} else if (
				...
				selection["residueTest"] = (l) => {
					if (
						...
						siftMappings = SIFTSMapping.Provider.get(
							contextData.models[0]
						).value;
						currentAccession = param.uniprot_accession!;
					}
				...
				selection["residueTest"] = (l) =>
					StructureProperties.residue.label_seq_id(l.element) ===
				...
				selection["residueTest"] = (l) => {
					const labelSeqId = StructureProperties.residue.label_seq_id(
						l.element
					);
					return (
						labelSeqId >= param.start_residue_number! &&
				...
				selection["residueTest"] = (l) => {
					const authSeqId = StructureProperties.residue.auth_seq_id(
						l.element
					);
					return (
						authSeqId >= param.start_auth_residue_number! &&
			...
			if (param.atoms) {
				selection["atomTest"] = (l) =>
					param.atoms!.includes(
						StructureProperties.atom.label_atom_id(l.element)
					);
			}

			if (param.atom_id) {
				selection["atomTest"] = (l) =>
					param.atom_id!.includes(
						StructureProperties.atom.id(l.element)
					);
			}
	...
	export function getInteractivityLoci(params: any, contextData: any) {
		const sel = StructureQuery.run(
			QueryHelper.getQueryObject(params, contextData) as any,
			contextData
		);
		return StructureSelection.toLociWithSourceUnits(sel);
	}

	export function getHetLoci(
		queryExp: Expression.Expression,
		contextData: any
	) {
		const query = compile<StructureSelection>(queryExp);
		const sel = query(new QueryContext(contextData));
...
export async function runWithProgressMessage(
	...
	const task = Task.create(progressMessage ?? "Task", async (ctx) => {
		...
		try {
			if (progressMessage) {
				setTimeout(() => {
					if (!done) ctx.update(progressMessage);
...
export function getStructureUrl(
	...
	const useStaticFile =
		request.queryType === "full" && !initParams.lowPrecisionCoords;
	if (useStaticFile) {
		const suffix =
			initParams.encoding === "bcif" ? ".bcif" : "_updated.cif";
...
export function addDefaults<T extends {}>(
